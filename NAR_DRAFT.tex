...
In addition, MuTect is particularly useful because it directly reports the log ratio of the of the likelihood functions for the null and alterative hypotheses in \ref{eqn:hypothesis} as either \texttt{TLOD} (MuTect1) or \texttt{t\_lod\_fstar} (MuTect2).
We have used MuTect 1.1.7 for all of the analysis in this paper, so the output of MuTect provides us with
...
\begin{equation}
  ...
  $$
\end{equation}
since $\mathrm{log}_{10}(\mathrm{logit}(3\cdot10^{-6}) \approx -6$.
...
The "Intermediate" profile is simulated with $\mathbf{\pi}$ set to an equal combination of the COSMIC signatures 1, 4, and 5, which has been associated with tobacco carcinogens and is common in lung cancers.
...
We utilized two real data sets where deep sequencing and variant validation were performed, with the specific purpose of evaluating tumor variant calling pipelines.
...
We estimated a mutation rate for this tumor of $3\cdot10^{-8}$, which is line with the mutation rates generally considered normal in AML.
...
We downloaded aligned reads for this tumor, and called variants with MuTect 1.1.7 as described in Materials \& Methods.
We estimated a mutation rate for this tumor of $3\cdot10^{-8}$, and the area under the precision-recall curve is identical (.972) for MuTect and \batcave.

These two data sets are among a very small number of tumors which have been sequenced at high depth, and for which variant validation was undertaken.
We report only precision-recall comparisons here because in both cases the "gold" standard data sets consist of variants that were "called" by one or more variant detection pipelines, and confirmed to be present or absent.
There is thus no way to compute true and false negative fractions for these data sets.
...
\begin{itemize}
  \item Every tumor is different. Even tumors of the same type have meaningfully different mutation profiles
