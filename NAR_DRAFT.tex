...
There are many aspects of local genomic architecture, generally described as the genomic \textit{context}, which can affect the local somatic mutation rate at a particular site in the genome.
Here we focus on the tri-nucleotide context, which has been shown to have a strong effect on the prior probability of mutation \citep{Nik-Zainal2012a,Alexandrov2015,Lee-Six2018}.
...
Figure \ref{NAR-sigfig} shows several examples of mutation profiles from both real and simulated tumors.
...
In an exponentially growing tumor, the number of new mutations per cell division is the product of the number of bases $N$, the per base mutation rate $\mu$, and the \textit{effective} birth rate $\beta$, which is a function of the birth and death rate.
...
The estimated effective mutation rate, $\mu/\beta$, is in the range of 3e-7 \bkmcomment{filling in today} for all simulated tumors.
We note that the effective mutation rate is lower than the simulated rate of 3e-7.
...
The heart of \batcave, the context-dependent prior probability of mutation, depends on the prior converging to a good representation of the data generating distribution within the set of high-confidence mutations.
...
\toprule
Depth & Signature & $\mu/\beta$ & AUROC &  & AUPRC & & ICI & 
\\
& & (estimated) & MuTect & \batcave & MuTect & \batcave & MuTect & \batcave
\\
\colrule
100X whole genome & Concentrated & 3.6e-7 & Row 1 & -- & -- & -- & -- & --
\\
100X whole genome & Intermediate & 3.2e-7 & Row 1 & -- & -- & -- & -- & --
\\
