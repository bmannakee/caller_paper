...
\begin{abstract}
Identifying somatic mutations withins tumors is key to understanding treatment resistance, patient prognosis, and tumor evolution.
Mutations at low allelic frequency, which are carried by only a portion of cells within a tumor, are particularly difficult to detect.
Many algorithms have been developed to identify such mutations, but none models a key aspect of tumor biology.
Namely, every tumor has its own profile of mutation types that it tends to generate.
We present \batcave (Bayesian Analysis Tools for Context-Aware Variant Evaluation), an algorithm that first learns the tumor mutational profile and mutation rate then uses it as a prior for evaluating potential mutations.
We also present an R implementation of the algorithm that is built on the popular somatic variant caller MuTect.
Using simulations, we show that adding the \batcave algorithm to MuTect improves variant identification.
It also improves the calibration of posterior probabilities, enabling a more principled tradeoff between precision and recall.
Lastly, we show that \batcave also performs well on real data.
