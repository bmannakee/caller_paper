...
The identification of low frequency mutations from Next Generation Sequencing data remains challenging.
In general the ability to reliably detect low frequency mutations increases as sequencing depth increases, yet false positive identification rates also increase with sequencing depth.
...
For example, Fig. \ref{NAR-sigfig} \textbf{a-c}, shows mutation profiles from several tumors.
...
\begin{figure}
  ...
  \rngcomment{Need more basic labels for each panel. E.g. ``Real breast tumor'', ``Real accumulated myeloid leukemia'', ``Simulated concentrated profile''}
  \end{center}
  \caption{Tumor mutation profiles.
  \rngcomment{Add explanation of how to read a profile. ``In each panel, the histograms show...''}
  \textbf{(a)} The observed mutation profile of a breast tumor \cite{Shi2018}.
  \textbf{(b)} The observed mutation profile of an acute myeloid leukemia \cite{Griffith2015}.
  \textbf{(c)} \rngcomment{Add a third (and possibly fourth) real tumor, with a more concentrated profile. So we can illustrate that our simulations aren't out of line with reality. Ideally two tumors of same type that have noticeably different profiles.}
...
\begin{equation}
  ...
  \end{array}
\end{equation}
The first two terms on the right-hand side can be estimated from the observed mutation profile (Fig.~\ref{NAR-sigfig}).
%Here $\mathrm{p}(\mathrm{S}_{m,c})$ represents probability that a substitution of type $\mathrm{S}_{m,c}$ will occur.

\rngcomment{Got here...}

We model the observed mutation profile $S$ (Fig.~\ref{NAR-sigfig}a-c) as multinomial with parameter $\boldsymbol{\pi} = \{\pi_{m,c}\}$.
