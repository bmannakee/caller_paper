...
Briefly, a loess-smoothed regression is fit by regressing the binary (T/F) variant classification against the reported posterior probability of a true variant for both MuTect and the Calibrated model (Figure \ref{NAR-fig1} \textbf{e}).
...
\bkmcomment{Honestly, just putting the R code here exactly as Austin does is probably the best explanation?}
...
\toprule
Experiment & $\mu$ & AUROC &  & AUPRC & & ICI & 
\\
& & MuTect & Calibrated & MuTect & Calibrated & MuTect & Calibrated
\\
\colrule
Row 1 & Row 1 & Row 1 & -- & -- &&&
\\
...
{AUROC/AUPRC = area under ROC/PRC curve. ICI = Integrated calibration index}
\end{table*}
