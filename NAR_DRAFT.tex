...
%For both whole exome and whole genome the prior converges to the data generating distribution easily within the number of available high-confidence mutations, which for the leukemia~\citep{Griffith2015} is over 17000, and for our simulated data sets ranges between 1500 and 5000.
  ...
  \begin{figure*}
    ...
    \rngcomment{Make \batcave lines thin, MuTect lines thick, and put MuTect in back in all comparison plots.}
}
  \label{NAR-wes_fig}
  \end{figure*}
...
\begin{figure}
  ...
  For realistic posterior probability thresholds,  positive predictive values are greater for \batcave than MuTect, no matter the concentration of the mutation profile.
  }
\label{NAR-ppv_fig}
\end{figure}

In practice, variant calling is a classification problem, in which a threshold score must be selected above which a variant call is made.
The proper threshold is ideally a choice the user makes based on their particular needs, which often balance precision with recall.
The posterior probability estimates provided by \batcave allow the user to select a tumor-specific threshold.
For posterior probability thresholds between 60 and 90\%, \batcave exhibits positive predictive values that are relatively close to the threshold (Fig.~\ref{NAR-ppv_fig}\&\ref{NAR-ppv_wgs_fig}).
In other words, selecting a minimum posterior probability creates a classifier with positive predictive value close to that threshold.
On the other hand, the posterior probabilities from MuTect provide a poor estimate of positive predictive value (Fig.~\ref{NAR-ppv_fig}\&\ref{NAR-ppv_wgs_fig}).
For any posterior probability above 70\%, MuTect has a false positive rate of roughly 8\%, whereas \batcave has a false positive rate that decreases as the posterior probability increases.
As a result, \batcave posterior probabilities provide a useful way to select tumor-specific thresholds. 
At any posterior probability threshold \batcave has recall better than MuTect.
...
The improved calibration of \batcave provides several advantages, including a tumor-specific means to select a threshold for positive classification that exhibits interpretable positive predictive value properties.
Cancer genomics for research and drug discovery is an important yet costly endeavour, and manual review of variants is often required to limit the number of false leads followed.
Improved calibration allows researchers to select a tolerable false positive rate, and focus efforts on a set of variants with known probability of being present.
In the clinic, identified variants act as biomarkers for susceptibility to targeted drugs, and well calibrated probabilities facilitate the use of risk models in the choice of treatment rather than an all or nothing approach.
\bkmcomment{I mean by "risk models", if you can assign a probability of the mutation being present you can make informed tradeoffs between likelihood of benefit and risk of harm for a particular targeted therapy.}
A properly calibrated model implies a known rate of false positives.
One reason why tumors are not routinely sequenced to high depth is that as sequencing depth increases, false positive rate increases without bound \citep{Cibulskis2013}.
The International Cancer Genome Consortium recommends that catalogues of somatic mutation target a true positive rate of 95\%, and an acquisition rate of 80\% of all mutations present in a sample \texttt{https://icgc.org/icgc/goals-structure-policies-guidelines/e8-genome-analyses}.
Assuming a sample size of 1 million cells, and a per-base mutation rate of $3 \dot 10^{-8}$, implies 10 million mutations per sample in a neutrally evolving tumor.
A naive application of Eqn. \ref{eqn:mut_rate} implies a minimum variant allele frequency of \bkmcomment{calculate this, will be very low.}, requiring sequencing depths far in excess of current standards.
Proper calibration will allow deeper sequencing without ballooning false positive rates.
\bkmcomment{we can talk about what should actually survive from this previous section. I like the specific example but may be too wordy here. One of the things I have tried to do is establish that ppv qualities arise from superior calibration, and that allows the use of calibration as short hand here in the discussion. This may not be well done.}
