...
\bkmcomment{I think there is a reasonable wording here in terms of positive predictive value.
Think of the threshold as the posterior class probability above which we set all probabilities to 1, and below to zero?
Getting better below but I still need to think harder.}
In practice, variant calling is a decision problem, in which a threshold score must be selected above which a variant call is made.
...
\batcave provides posterior probabilities that are well calibrated when averaged not only across the whole range of possible probabilities, but in particular at higher probability ranges applicable in most situations.
For posterior probability thresholds between 60 and 90\%, \batcave exhibits substantially better average calibration than MuTect (Fig.~\ref{NAR-ppv_fig}\&\ref{NAR-ppv_wgs_fig}).
...
The improved calibration of \batcave provides several advantages.
