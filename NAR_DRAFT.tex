...
Each simulate profile includes a contribution from COSMIC mutation signature 1 which is found in nearly all tumors and is associated with spontaneous cytosine deamination.
...
For any probability between .6 and .95 \batcave is better calibrated than Mutect.
Essentially, for any posterior probability above .7 MuTect has a false positive rate around 8\%, which \batcave has a false positive rate that increases as posterior probability decreases.
The cost of this compression is recall.
...
The area under the precision-recall curve is essentially identical (.995 \& .996) for MuTect and \batcave (Figure \ref{NAR-wes_fig} \textbf{a)} and Table \ref{table:01}).
...
\begin{figure}
  ...
  Note the restricted axes to improve readability of points.\bkmcomment{how do called allele frequency distributions differ?}}
\label{NAR-ppv_fig}
\end{figure}
...
\bkmcomment{Discuss assumption that vaf in uniform. opportunity for further work.}
