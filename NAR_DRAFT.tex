...
\begin{figure}
  ...
  \end{center}
  \caption{Calibration of posterior probability for 500X exomes. 
  For realistic thresholds, posterior probabilities are more accurately calibrated for \batcave than MuTect, no matter the concentration of the mutation profile.
  \rngcomment{Need to be careful about what is represented here. Here precision and recall are for all variants \textbf{above} the threshold, correct?}
  }
\label{NAR-ppv_fig}
\end{figure}

In practice, variant calling is a decision problem, in which a threshold must be selected above which we accept a variant call.
The proper threshold is ideally a choice the user makes based on their particular needs.
...
At any posterior probability \batcave returns a larger fraction of all positive calls than MuTect. \rngcomment{What does this mean?}
