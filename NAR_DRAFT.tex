...
The 3-nucleotide context at a particular base is made up of the identity of the reference base, the identity of the alternate allele, and the identity of the 3` and 5` flanking bases.
There are four possible bases 3` and 5` of the site, leading to 16 possible flanking base combinations.
Folding the central bases to the pyrimidines leaves 2 possible reference bases at the site, and three possible alternate alleles for each.
Thus, there are $2 \cdot 3 \cdot 16 = 96$ possible combinations of flanking bases, reference bases, and alternate alleles (see Figure \ref{NAR-sigfig}).
The mutation context $C$ is indexed by $i=\{1 \dots 16\}$ flanking base combinations, $j = \{1,2\}$ reference bases, and $j = \{1,2,3\}$ alternate alleles.
We model $ p(C | M) $ as a multinomial distribution with parameter $ \boldsymbol{ \pi } = \{\pi_{i,j,k}\} $ and pseudo-count hyper-parameter $\boldsymbol{\alpha}$
...
\end{array}
$$
We use high confidence variant calls -- odds in favor of the mutation greater than 10:1 -- to estimate $\boldsymbol{\pi}$.
...
By selecting $\alpha$ such that we are highly confident in all calls at frequencies greater than $\alpha$, we can compute $\hat{\mu}$.
In all results here we select $\alpha = 0.05$.
...
However, $p(m,M)$ is not constant for every site in the genome, and follows a distribution -- conditional on the genomic context surrounding the site -- described by the mutation signature of the tumor and denoted here $p(m,M | C)$ \citep{Nik-Zainal2012a,Alexandrov2015,Lee-Six2018}.
The prior probability $\mathrm{p}(m,M \mid C)$ can be decomposed as
$$
\begin{array}{rl}
  \mathrm{p}(m,M \mid C) = &\mathrm{p}(m \mid C) \mathrm{p}(M \mid C) \\ \\ 
  = &\mathrm{p}(m \mid C) \mathrm{p}(C \mid M)\frac{p(M)}{p(C)}
\end{array}
$$
...
The estimated mutation signature and mutation rate, allow us to compute $p(m,M | C)$ as 

$$
\begin{array}{rl}
  \mathrm{p}(m \mid C) = & \pi_{i,j,k} \\ \\
  \mathrm{p}(C \mid M) = & \sum_{k}\pi_{i,j} \\ \\
  \mathrm{p}(M) = & \hat{\mu},
\end{array}
$$
with $\mathrm{p}(C)$ the fraction of all sites in the genome with context $C_{i,j}$ divided by 3.
...
Plotting the smoothed regression predictions against the actual predictions would result in a diagonal line, and the Integrated Calibration Index is the average distance between the result and a diagonal line.

\begin{figure}[t]
\begin{center}
\includegraphics{figures/signature_figure.pdf}
\end{center}
\caption{Mutation signatures. 
\textbf{(a)} The observed mutation signature of the AML31 primary tumor. 
\textbf{(b)} A mutation signature used for simulating tumors, made up of equal proportions of COSMIC mutation signatures 1, 7, \& 11.
\textbf{(c)} Equal proportions of signatures 1, 4, \& 5.
\textbf{(d)} Equal proportions of signatures 1, 3, \& 5.
KL divergence between the estimated signature and the simulated mutation signature for \textbf{e)} Whole exomes and \textbf{f)} whole genomes.
The estimated mutation signature converges quickly to the target as the number of mutations evaluated rises.
}
\label{NAR-sigfig}
\end{figure}
...
The rate and degree of convergence are correlated with the concentration of the mutation signature operating in the tumor (Figure \ref{NAR-sigfig}\textbf{e,f}).
\subsection{Performance on real data}

The Calibrated model has better AUROC and AUPRC than the MuTect model in this real tumor (Figure \ref{NAR-aml}\textbf{a,c}, Table \ref{table:01}).
The Calibrated model has slightly worse calibration as measure by ICI, but both models substantially underestimate the true positive probability throughout the range of possible values (Figure \ref{NAR-aml} \textbf{b,d}).
One caveat to consider when considering these results is that while AML31 was subject to extensive variant calling validation, their list of true positive calls likely does not include every true variant at low allele frequencies, and true negative calls were not evaluated.
...
The Calibrated model tends to score true negative variants lower and true positive variants higher than Mutect (Figure \ref{NAR-fig2} \textbf{b,d}).
The Integrated calibration index for the Calibrated model is better than that of the MuTect model for all tumors simulated.
...
\\
\colrule
100X whole genome & Concentrated & Row 1 & Row 1 & -- & -- & -- & -- & --
\\
100X whole genome & Intermediate & Row 1 & Row 1 & -- & -- & -- & -- & --
\\
100X whole genome & Diffuse & Row 1 & Row 1 & -- & -- & -- & -- & --
\\
500X whole exome & Concentrated & Row 1 & .848 & .928 & .674 & .757 & .0585 & .0465
\\
500X whole exome & Intermediate & Row 1 & .847 & .842 & .677 & .707 & .0587 & .0566 
\\
500X whole exome & Diffuse & Row 1 & .850 & .873 & .676 & .698 & .0565 & .0575
\\
