...
We downloaded the primary and tumor bam files generated for the whole genome sequences in \cite{Griffith2015}, from dbGaP accession number phs000159.v8.p4.
...
The whole exome sequencing bam files generated by \cite{Shi2018} were downloaded from NCBI SRA no. SRP070662.
...
\subsection{Calibration metric}
	...
	Plotting the smoothed regression predictions against the actual predictions would result in a diagonal line, and the Integrated Calibration Index is the average area between the loess fit and a diagonal line.
...
The \batcave algorithm focuses on the tri-nucleotide context and substitution type, but there are many other types of \textit{genomic context} which can effect local mutation rates \cite{Buisson2019}.
These other types of context could be very useful in computing a local or regional mutation rate, as opposed to the genome-wide but tumor-specific mutation rate used by \batcave.
