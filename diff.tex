...
%DIF DEL NAR_DRAFT_FirstSubmission.tex   Fri Jan 10 15:17:31 2020
%DIF ADD NAR_DRAFT.tex                   Mon Jan 13 10:53:14 2020
...
%DIF 19-23c19-23
%DIF < \copyrightyear{2008}
%DIF < \pubdate{31 July 2009}
%DIF < \pubyear{2009}
%DIF < \jvolume{37}
%DIF < \jissue{12}
%DIF -------
...
\DIFdelbegin \DIFdel{Folding the central base to the pyrimidines, there are two possible bases at the focal site, and there are four possible bases 3' and 5' of the focal site, yielding $2 \cdot 4 \cdot 4$ possible tri-nucleotide contexts $C$.
At the focal site, a mutation $m$ can be to any of three alternate alleles.
}\DIFdelend \DIFaddbegin \DIFadd{There are six classes of base substitution: C$>$A, C$>$G, C$>$T, T$>$A, T$>$C, T$>$G (all referred to by the pyrimidine of the mutated Watson-Crick pair).
This results in a total of 32 ($4\times 2 \times 4$) reference contexts and 3 alternate bases per reference base.
%DIF > Folding the central base to the pyrimidines, there are two possible bases at the focal site, and there are four possible bases 3' and 5' of the focal site, yielding $2 \cdot 4 \cdot 4$ possible tri-nucleotide contexts $C$.
%DIF > At the focal site, a mutation $m$ can be to any of three alternate alleles.
}\DIFaddend Indexing by the $c=\{1 \dots 32\}$ contexts and by the $m = \{1 \dots 3\}$ alternate bases, we have 96 possible substitution types $S_{m,c}$.
...
\DIFaddbegin \DIFadd{Sample contamination by normal cells can result in underestimation of tumor mutation allele frequencies.
...
}\DIFaddend We set $f_{max}$ to be the largest allele frequency in $\mathrm{D}$, but we must choose $f_{min}$ conservatively, depending on sequencing depth.
In the R implementation of \batcave, $f_{min}$ \DIFdelbegin \DIFdel{is a free parameter}\DIFdelend \DIFaddbegin \DIFadd{and $p_{ur}$ are free parameters}\DIFaddend .
For this paper, we set $f_{min} = 0.05$, because we are working at high depth.
\DIFaddbegin \DIFadd{For our simulated data analyses, we set $p_{ur}=1$, and for our real data analyses, we set $p_{ur}$ to the estimated purity.
}


\DIFaddend %A value 0.12 for $f_{min}$ at lower sequencing depth is suggested in Williams et al., 2016~\cite{Williams2016}.
...
We have implemented the \batcave algorithm as an R package \DIFaddbegin \DIFadd{called }\DIFaddend \texttt{batcaver}.
...
\DIFaddbegin \DIFadd{An assumption of the }\batcave \DIFadd{algorithm is that the mutation profile of low-confidence mutations is similar to that of the high-confidence mutations from which the prior is inferred.
To test this assumption, }\texttt{\DIFadd{batcaver}} \DIFadd{performs a statistical hypothesis test comparing these two mutation profiles \mbox{%DIFAUXCMD
\cite{Plunkett2019}}\hspace{0pt}%DIFAUXCMD
.
If the two profiles differ at a significance threshold of $\alpha=0.05$, }\texttt{\DIFadd{batcaver}} \DIFadd{issues a warning and outputs the profiles.
%DIF > The user can then assess them and follow-up on any biologically interesting differences.
}
...
\DIFaddbegin \DIFadd{Griffith et al.\ estimated the purity of their primary sample to be 90.7\%, and we used this value for tumor purity in }\texttt{\DIFadd{batcaver}}\DIFadd{.
}\DIFaddend Shi et al.\ generated a gold set of variant calls for each tumor region sequenced~\cite{Shi2018}, which we used for our true positive dataset.
For these multi-region data, we \DIFdelbegin \DIFdel{ran }\DIFdelend \DIFaddbegin \DIFadd{analyzed the three biological replicates from each of six tumors (Table~\ref{table:01}), running }\DIFaddend \batcave separately on each \DIFdelbegin \DIFdel{sequenced region and }\DIFdelend \DIFaddbegin \DIFadd{of the eighteen samples, and we }\DIFaddend combined results to generate precision-recall curves.
\DIFaddbegin \DIFadd{Shi et al.\ estimated sample purities of between 26\% and 80\%, and we used these estimates in }\texttt{\DIFadd{batcaver}}\DIFadd{.
}\DIFaddend We called variants using Mutect 1.1.7 as in our simulations, except that both these data sets were originally aligned to \DIFdelbegin \DIFdel{GRCr37}\DIFdelend \DIFaddbegin \DIFadd{GRCh37}\DIFaddend , so we used that reference.
...
When applied to simulated data, the prior converged within a few hundred mutations (Fig.~\ref{NAR-kl_fig})\DIFaddbegin \DIFadd{, and the }\texttt{\DIFadd{batcaver}} \DIFadd{package emits a warning if there are fewer than 500 high-confidence mutations}\DIFaddend .
...
\begin{table*}[b]
  \DIFdelbeginFL %DIFDELCMD < \tableparts{%
%DIFDELCMD <   \caption{Variant calling metrics for all data sets.}
%DIFDELCMD <   %DIFDELCMD < \label{table:01}%%%
%
%DIFDELCMD <   }{%
%DIFDELCMD <   \begin{tabular}{lllllllllll}
%DIFDELCMD <   \toprule
%DIFDELCMD <   Scenario & Mutation profile & $\mu$ & \multicolumn{2}{c}{AUROC} & \multicolumn{2}{c}{AUPRC} & \multicolumn{2}{c}{ICI} 
%DIFDELCMD <   \\
%DIFDELCMD <   & & (estimated) & MuTect & \batcave & MuTect & \batcave & MuTect & \batcave
%DIFDELCMD <   \\
%DIFDELCMD <   \colrule
%DIFDELCMD <   100X whole genome & Concentrated & 3.6e-7 & .987 & .993 & .972 & .975 & .117 & .287
%DIFDELCMD <   \\
%DIFDELCMD <   100X whole genome & Intermediate & 3.2e-7 & .987 & .989 & .972 & .973 & .118 & .214
%DIFDELCMD <   \\
%DIFDELCMD <   100X whole genome & Diffuse & 3.2e-7 & .988 & .989 & .971 & .973 & .120 & .219
%DIFDELCMD <   \\
%DIFDELCMD <   500X whole exome & Concentrated & 3.6e-7 & .848 & .929 & .674 & .758 & .138 & .109
%DIFDELCMD <   \\
%DIFDELCMD <   500X whole exome & Intermediate & 3.6e-7 & .847 & .881 & .677 & .706 & .108 & .112 
%DIFDELCMD <   \\
%DIFDELCMD <   500X whole exome & Diffuse & 3.6e-7 & .850 & .873 & .676 & .698 & .105 & .116
%DIFDELCMD <   \\
%DIFDELCMD <   real AML \cite{Griffith2015} & Actual & 3.6e-8 & -- & -- & .995 & .996 & -- & --
%DIFDELCMD <   \\
%DIFDELCMD <   real breast \cite{Shi2018} & Actual & 3.6e-8 & -- & -- & .972 & .972 & -- & --
%DIFDELCMD <   \\
%DIFDELCMD <   \botrule
%DIFDELCMD <   \end{tabular}%
%DIFDELCMD <   }
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \tableparts{%
  ...
  \DIFaddendFL {$\mu$ = per-base mutation rate, AUROC/AUPRC = Area Under Receiver Operating Characteristic / Precision-Recall Curve, ICI = Integrated Calibration Index. Smaller values of ICI are superior.}
 \end{table*}
...
We estimated a per-base mutation rate for this tumor of \DIFdelbegin \DIFdel{$4\cdot10^{-8}$}\DIFdelend \DIFaddbegin \DIFadd{$3.6\times10^{-8}$}\DIFaddend , which is consistent with previous estimates of AML mutation rates \cite{Griffith2015,Williams2018}.
For \DIFaddbegin \DIFadd{this tumor, the mutation profiles of high- and low-confidence mutations differ ($p \sim 10^{-12}$).
The low-confidence mutation profile contains additional spikes of mutations corresponding to five substitution types (Fig.~\ref{fig:AML_profiles}).
Nevertheless, for }\DIFaddend both MuTect and \batcave, the precision-recall curve is almost perfect for the validated variants (\DIFdelbegin \DIFdel{.995 }\DIFdelend \DIFaddbegin \DIFadd{.988 }\DIFaddend \& .996 area under the curve) (Fig.~\ref{NAR-wes_fig}E and Table~\ref{table:01}).
...
We estimated an average per-base mutation rate for these tumor regions of \DIFdelbegin \DIFdel{$4\cdot10^{-8}$}\DIFdelend \DIFaddbegin \DIFadd{$6.6\times10^{-7}$}\DIFaddend , which is consistent with observed mutation rates for breast cancers \cite{Alexandrov2019} and with the low number of validated somatic mutations.
...
\DIFaddbegin \DIFadd{Following MuTect \mbox{%DIFAUXCMD
\cite{Cibulskis2013}}\hspace{0pt}%DIFAUXCMD
, }\batcave \DIFadd{assumes a uniform prior on the variant allele frequency.
...
Note however, that if the goal were not to best identify individual variants, but rather to best estimate the distribution of variant allele frequencies within the tumor, then a nonuniform frequency prior would be appropriate~\mbox{%DIFAUXCMD
\cite{Nielsen2012}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFadd{To estimate the tumor mutation rate, }\batcave \DIFadd{uses a model of the distribution of variant allele frequencies  generated during exponential growth (Eq.~\ref{eqn:mut_rate}).
...
Nevertheless, a potential extension of the }\batcave \DIFadd{algorithm would be to incorporate external copy number calls in order to adjust allele frequencies before estimating the mutation rate.
}

\DIFadd{Sequencing multiple regions of the same tumor is of growing interest for understanding tumor heterogeneity and evolution \mbox{%DIFAUXCMD
\cite{McGranahan2017}}\hspace{0pt}%DIFAUXCMD
.
In fact, the breast cancer data we analyze came from a multi-region study~\mbox{%DIFAUXCMD
\cite{Shi2018}}\hspace{0pt}%DIFAUXCMD
.
...
In the future, the }\batcave \DIFadd{algorithm could be integrated into a multi-sample caller such as MultiSNV~\mbox{%DIFAUXCMD
\cite{Josephidou2015}}\hspace{0pt}%DIFAUXCMD
.
...
\DIFaddend The \batcave algorithm is computationally inexpensive; our current implementation adds 1 second per 22,000 variants evaluated to a standard GATK best-practices variant calling pipeline.
...
\DIFaddend An  implicit assumption of \DIFdelbegin \DIFdel{our approach }\DIFdelend \DIFaddbegin \batcave \DIFaddend is that the mutational process does not change between high- and low-confidence variants, implying that the mutational profile of the tumor is temporally constant.
Recent studies have found differences in mutational profiles among variants of different allelic frequencies~\cite{Rubanova2018a}, \DIFdelbegin \DIFdel{although }\DIFdelend \DIFaddbegin \DIFadd{and although in general }\DIFaddend those differences are relatively small\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, they can be significant in some cases.
Our }\texttt{\DIFadd{batcaver}} \DIFadd{software warns the user if the high- and low-confidence mutational profiles are statistically different.
It also outputs those profiles, so the user can assess whether the difference will substantially affect their results.
}\DIFaddend A potential extension of the \batcave algorithm is to process potential variants in order of descending allelic frequency and to update the estimated mutational prior as the algorithm proceeds.
...
%DIF <  \bkmcomment{I looked at a bunch of NAR methods papers, and none include a conclusions section, so I moved it to the last paragraph of discussion.}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section{Software Availability}
...
\centering
  ...
  \caption{\DIFaddFL{Mutation profiles of A) high-confidence and B) low-confidence mutations in the AML data.}}
...
\begin{table*}[h]
  ...
  {\DIFaddFL{Shi et al. did not estimate purity for Case 2 biorep C. We chose 40\% purity, based on the other biological replicates from this case.}}
\end{table*}
\DIFaddend 
